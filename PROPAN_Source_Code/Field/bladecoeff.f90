...
!-----------------------------------------------------------------------------------------------!
SUBROUTINE BLADECOEFF(TT,N1,N2,XTMP,YTMP,ZTMP,PTMP)
!-----------------------------------------------------------------------------------------------!
...
DOUBLE PRECISION :: PHIS,PHID,POT(NCP,NRP)
...
!-----------------------------------------------------------------------------------------------!
                     ...
                     ELSEIF (((I == 1).OR.(I == NC).OR.(I == NC+1).OR.(I == NCP)) &
                                                         .AND.(J == NRP).AND.(ISTRIP == 2)) THEN
                        XT(1:3)=XX(1:3)
                        YT(1:3)=YY(1:3)
                        ZT(1:3)=ZZ(1:3)
                        CALL GAUSSPAN3(4,XT,YT,ZT,XTMP(II,JJ),YTMP(II,JJ),ZTMP(II,JJ),PHIS,PHID)
...
!-----------------------------------------------------------------------------------------------!
               IF (KB == 1) THEN
                  PTMP(II,JJ)=PTMP(II,JJ)+PHID*POTP(I,J,TT)/(4.D0*PI)
               ELSE
                  PTMP(II,JJ)=PTMP(II,JJ)+PHID*POT (I,J   )/(4.D0*PI)
               END IF
               PTMP(II,JJ)=PTMP(II,JJ)+PHIS*SOURCEP(I,J,KB)/(4.D0*PI)
!-----------------------------------------------------------------------------------------------!
            END DO !JJ=1,N2
         END DO !II=1,N1
!-----------------------------------------------------------------------------------------------!
