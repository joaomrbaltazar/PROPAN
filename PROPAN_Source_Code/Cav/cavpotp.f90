...
!-----------------------------------------------------------------------------------------------!
      S1=DSQRT(AT1XP(IDP(J,TT)+1,J)**2+AT1YP(IDP(J,TT)+1,J)**2+AT1ZP(IDP(J,TT)+1,J)**2)
      S2=DSQRT(AT1XP(IDP(J,TT)+2,J)**2+AT1YP(IDP(J,TT)+2,J)**2+AT1ZP(IDP(J,TT)+2,J)**2)+S1*2.D0
      S3=DSQRT(AT1XP(IDP(J,TT)+2,J)**2+AT1YP(IDP(J,TT)+2,J)**2+AT1ZP(IDP(J,TT)+2,J)**2)+S2
      S3=DSQRT(AT1XP(IDP(J,TT)+3,J)**2+AT1YP(IDP(J,TT)+3,J)**2+AT1ZP(IDP(J,TT)+3,J)**2)+S3
!-----------------------------------------------------------------------------------------------!
      A0=S2*S3/(S1-S2)/(S1-S3)
      A1=S1*S3/(S2-S1)/(S2-S3)
      A2=S1*S2/(S3-S1)/(S3-S2)
      POT1=A0*POTP(IDP(J,TT)+1,J,TT)+A1*POTP(IDP(J,TT)+2,J,TT)+A2*POTP(IDP(J,TT)+3,J,TT)
!-----------------------------------------------------------------------------------------------!
...
!-----------------------------------------------------------------------------------------------!
            ...
            IF (TT > 0) THEN
               IF (TT == 1) THEN
                  DPHIDT=POTPWP(I,J2,1)/DTETA-POTPWP(I,J2,0)/DTETA
               ELSE !(TT == 1)
                  DPHIDT=POTPWP(I,J2,TT-2)*0.5D0/DTETA- &
                         POTPWP(I,J2,TT-1)*2.0D0/DTETA+ &
                         POTPWP(I,J2,TT  )*1.5D0/DTETA
               END IF !(TT == 1)
               IF (TT <= (2*NTETA)) THEN
                  FRELAX=CREX*DFLOAT(TT-1)/DFLOAT(2*NTETA-1)
               ELSE !(TT <= (2*NTETA))
...
!-----------------------------------------------------------------------------------------------!
            POT2           =POT1+WORK1-WORK2
            POTPWP(I,J2,TT)=(POT1+POT2)*0.5D0
            POT1           =POT2
         END DO !I=IDPWP(J,TT),IRPWP(J,TT)
      END IF !(IDPWP(J2,TT) /= 0)
   END IF !(J2 > 0)
END DO !J=1,NRP
!-----------------------------------------------------------------------------------------------!
END SUBROUTINE CAVPOTP
!-----------------------------------------------------------------------------------------------!
